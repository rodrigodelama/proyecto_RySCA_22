...
int set_gateway(ipv4_addr_t next_hop)
{
    ...
    {
        return 0; // si sale 0, la gateway ha de ser la IP de donde lo hemos recibido
    }
    
    return 1; // si no es igual a "0.0.0.0" , la gateway ha de ser next_hop
}
    ...
    while (1)
    {
        ...
        //VALID ROUTE UPDATE
            ...
            for(int i = 0; i < numero_de_vectores_distancia; i++)
            {
                ...
                {
                    is_our_route = 1;
                    timerms_t * index_timer = & rip_table->routes[i]->timer_ripv2;
            ...
            for(int i = 0; i < numero_de_vectores_distancia; i++)
            {
                ...
                if(route_index == -1) //We DONT have the route in our ripv2_table
                {
                    ...
                    if(metric_rcvd < 16)
                    {
                        ...
                        if(calc_gateway == 0)
                        {
                            //addr recieved as next hop is 0.0.0.0, we will use the source ip
                ...
                //We HAVE the route
                    ...
                    if(set_gateway(ripv2_msg->vectores_distancia[i].next_hop) == 0) // si es igual a 0 Gateway shall be 
                    {
