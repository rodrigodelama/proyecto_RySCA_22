...
#define RIPv2_DISTANCE_VECTOR_SIZE 20
...
typedef struct ripv2_route {
    ...
    ipv4_addr_t gateway_addr; //siguiente salto, si esta en mi subnet, este campo es 0.
    uint32_t metric ;
...
typedef struct vector_distancia //vectores distancia
{
    ...
    uint16_t etiqueta_ruta; // tambien es 0
...
typedef struct ripv2_msg
{
    ...
    vector_distancia_t vectores_distancia[25]; //(20 bytes) es el tamaño de cada "vector distancia"
                    //solo pueden haber de 25 vectores distancia en un mensaje RIPv2
} ripv2_msg_t;
