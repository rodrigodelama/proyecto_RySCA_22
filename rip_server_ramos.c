...
int main(int argc, char ** argcv) {
 ...
 capa_udp=udp_open(puerto_servidor,mifichero, fichero_tabla_renvio);

    unsigned char buffer_rip[LEN_PAYLOAD_RIP]; //1472 de capacidad
    uint16_t  puerto_cliente;
    ipv4_addr_t ip_router_que_recibimos;
    rip_route_table_t * tabla_rip = rip_route_table_create(); //Creamos la tabla de rutas
    ...
    //RECIBIMOS DURANTE EL MENOR TIEMPO DE LA TABLA
     ...
     datos_recibidos=udp_recv(capa_udp, &puerto_cliente, ip_router_que_recibimos, buffer_rip, LEN_PAYLOAD_RIP,time_rcv);
     ...
     rip_route_t  * ruta_actual = rip_route_table_get(tabla_rip,index_min);
        ...
        if(index_ruta==-1){
         ...
         int metrica_recibida = ntohl(paquete_recibido->entry_rip[i].metrica) +1;
          ...
          if(metrica_recibida<16){    //Añadimos la ruta a la tabla
           rip_route_t * ruta_nueva = (rip_route_t *) malloc(sizeof(struct rip_route));
...
//                              ----------------YA TENEMOS LA RUTA---------------------------
    ...
    /***********************SI QUIEREN QUE LE ENVIEMOS LA TABLA ********************/
      ...
      for(i = 0; i<numero_rutas; i++){
       ...
       udp_send(capa_udp, puerto_cliente, ip_router_que_recibimos, (unsigned char *) &paquete_rip, total_longitud);
